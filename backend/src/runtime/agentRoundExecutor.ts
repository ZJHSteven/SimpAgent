...
export class AgentRoundExecutor {
  constructor(private readonly providerClient: UnifiedProviderClient, private readonly traceSink?: AgentRoundTraceSink) {}
  ...
  async executeRound(
    ...
    for await (const event of this.providerClient.stream({ ...req, stream: true })) {
      rawEventsCount += 1;

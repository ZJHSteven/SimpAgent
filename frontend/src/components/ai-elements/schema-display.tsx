...
export const SchemaDisplayPath = ({
  ...
  // 这个组件需要用 innerHTML 高亮路径参数。
  // React 的 children 类型很宽，可能是数字、节点或数组；innerHTML 只能接收字符串。
  // 因此这里把非空 children 明确转成字符串，避免严格类型检查失败。
  const html = children == null ? highlightedPath : String(children);

...
async function expectComposerFocusStyleIsClean(page, editor) {
  // 先记录未聚焦时外层 composer 的原始阴影。
  // 这一步用于保证 focus 后不会被我们额外加成绿色边框或绿色内阴影。
  const surface = page.locator('.composer-surface-local')
  const initialSurfaceBoxShadow = await surface.evaluate(
    (element) => getComputedStyle(element).boxShadow,
  )

  // 用户反馈的问题发生在“点击输入框并开始输入”之后，所以测试也必须覆盖有文字时的 focus 状态。
  await editor.focus()
  await editor.fill('焦点视觉检查')

  // textarea 自己不能绘制 outline 或 box-shadow。
  // outline 会变成明显焦点环；box-shadow 会被圆角裁切成上下两条蓝线。
  ...
  // focus 后外层 composer 应保持原来的阴影，不新增上一版引入的绿色描边。

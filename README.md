...
- `POST /conversations`：创建会话，可传 `entryNodeId` 指定入口 agent。
- `GET /conversations`：列出轻量会话目录。
- `GET /conversations/:id`：查询默认聊天视图，不默认展开完整 trace。
- `POST /conversations/:id/fork`：从某条消息分叉会话。
- `PATCH /messages/:id`：修改历史消息，并创建新的 conversation 分支。
- `POST /conversations/:id/runs`：启动一次 agent turn。
...
- `GET /events/:id`：开发调试接口，读取单个 event payload。
- `GET /preset/export`：按表 JSON 导出当前定义层 preset。
- `POST /preset/reset`：清空 SQLite 并用 server 默认 preset 重建。

最小调用示例：

```bash
curl -X POST http://localhost:8788/conversations `
  -H "content-type: application/json" `
  -d "{\"title\":\"本地测试\"}"
```

Windows PowerShell 里可以继续用上一步返回的真实 `conversation.id` 启动 run：

```bash
curl -X POST http://localhost:8788/conversations/<conversation-id>/runs `
  -H "content-type: application/json" `
  -d "{\"input\":\"请用一句话介绍当前项目\"}"
```
...
- 后端 API 已切到 `conversation` 命名；旧前端仍有 `thread` 命名调用，下一轮需要跟随后端接口迁移。
...
- HTTP Server 的默认 3-agent preset、conversation API、事件调试接口和 preset 导出。
...
第一版已经建立 `nodes`、`edges` 和各类 payload tables。conversation、message、event、agent、tool、prompt unit、tag、workflow 都先是 node；tag 绑定不再使用专门关系表，而是写成 `edges.edge_type = "has_tag"`。

server 启动时会导入默认 3-agent preset：Agent A 可发现 B/C，Agent B 只能发现 A，Agent C 只能发现 B。preset 的通用导入、导出和 reset 能力在 `agent-core`，server 只保存默认资产。
...
- 当前 Vitest 会通过 workspace package 读取已构建输出；改动 `agent-core` 后先跑 `npm run build`，再跑 `npm test`，避免测试读到旧 `dist`。

...
第一版已经建立 `conversations`、`nodes`、`edges`、`events` 和 payload tables。当前 agent loop 仍通过 `TraceStore` 抽象写入，但底层会拆成 conversation、message、event、llm call、tool call 和 approval 记录。

...
5. [ ] **清理与验证**
   - 删除 `frontend/src/styles/chatgpt-compat.css` 引入和旧业务样式。
   - 更新 Playwright 用例，改用 role、文本、可访问名称和稳定组件行为断言。
   - 执行前端 lint、tsc、build、E2E 和根项目 typecheck/build/lint/test。
   - 用真实 Chromium 检查桌面/移动截图、console/pageerror 和横向溢出。

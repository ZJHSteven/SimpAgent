...
## 执行步骤
1. 从 `tem.html` 中提取可复用结构特征：侧栏、消息宽度、用户气泡、助手正文、底部 composer。
2. 将臃肿的线上快照替换为本地可运行的静态页面，避免依赖 ChatGPT 的 CDN、React hydration 和实验配置。
3. 写入教学向中文注释，保持单文件直观可读，方便初学者从上到下阅读。
4. 增加轻量 JS：发送消息、空输入拦截、Enter/Shift+Enter 行为、textarea 高度自适应、错误提示。
5. 执行静态语法检查、文本断言、浏览器渲染截图和基础交互测试，覆盖桌面与移动视口。

...
- `edges` 保存定义层 node 与 node 之间的关系。tag 绑定、workflow 边界、workflow 顺序、handoff、prompt 绑定、模型绑定都走 edge。
- event 的运行归属、父子、因果和前后顺序直接写在 `events` payload 字段里，不再为每个运行事件额外创建 event edge。
...
agent 节点的专属 payload。`prompt_binding_json` 记录 prompt unit 的绑定方式，`provider_strategy_node_id` 直接引用模型路由子表。

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `node_id` | TEXT PRIMARY KEY | 是 | 指向 `nodes.id`。 |
| `prompt_binding_json` | TEXT | 是 | prompt unit 绑定配置，JSON 字符串。 |
| `tool_policy_json` | TEXT | 否 | 工具可见性与默认审批策略。 |
| `provider_strategy_node_id` | TEXT | 否 | 指向 `provider_strategies.node_id`，作为默认模型路由策略。 |
...
| `actor_node_id` | TEXT | 否 | 执行该事件的 node，例如 agent / tool / workflow。用户直接输入时可为空。 |
| `parent_event_node_id` | TEXT | 否 | 父事件，例如 tool call 属于某次 agent invocation。 |
| `caused_by_event_node_id` | TEXT | 否 | 触发该事件的原因事件，例如 tool approval 由 tool call 触发。 |
| `previous_event_node_id` | TEXT | 否 | 同一 conversation 内上一条事件，用于线性回放。 |
...
| `provider_strategy_node_id` | TEXT | 否 | 指向 `provider_strategies.node_id`。 |
